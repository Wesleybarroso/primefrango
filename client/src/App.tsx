...
function Integracoes({ onNotice }: { onNotice: (message: string) => void }) { const connections = [["Mapas e distância", "Endereço, raio de entrega e rota"], ["Pagamentos", "Confirmação segura e webhooks"], ["Logística", "Cotação, despacho e rastreamento"], ["WhatsApp", "Eventos e suporte humano"], ["E-mail", "Confirmação de conta e pedido"], ["Assistente de IA", "Dúvidas e encaminhamento"]]; return <section className="page-grid integrations-page"><div className="integration-intro"><Settings2 size={24} /><div><h2>Conexões seguras</h2><p>As chaves permanecem mascaradas e são armazenadas no ambiente protegido do servidor.</p></div></div><div className="integration-grid">{connections.map(([title, text]) => <article key={title}><ShieldCheck size={18} /><h3>{title}</h3><p>{text}</p><button className="outline-button" type="button" onClick={() => onNotice(`${title}: configuração aguardando credenciais oficiais.`)}>Configurar</button></article>)}</div></section>; }
