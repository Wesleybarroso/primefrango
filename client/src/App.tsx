...
function MenuPage({ go }: { go: (view: PublicView) => void }) {
  const promotionsQuery = trpc.promotions.publicList.useQuery(undefined, { retry: false, refetchOnWindowFocus: false });
  return <main className="content-page menu-page"><div className="page-intro"><p className="eyebrow">CARDÁPIO</p><h1>Escolha seus itens e ajuste o carrinho.</h1><p>Produtos, promoções e disponibilidade serão configurados pelo painel administrativo. Esta visão preserva uma compra clara, sem inventar preço ou estoque.</p></div>{promotionsQuery.data?.map((promotion) => { const images = [promotion.image1Url, promotion.image2Url, promotion.image3Url].filter((url): url is string => Boolean(url)); return <section className="menu-promotion-banner menu-promotion-banner-v2" key={promotion.id}>{images.length ? <div className="promotion-public-gallery menu">{images.map((url, index) => <img key={url} src={url} alt={`${promotion.title} — imagem ${index + 1}`} />)}</div> : <Tags size={19} />}<div><p className="eyebrow">{promotion.badge || "PROMOÇÃO ATIVA"}</p><h2>{promotion.title}</h2><p>{promotion.description}</p><div className="menu-promotion-price">{promotion.salePriceCents ? <><s>{formatPrice(promotion.originalPriceCents)}</s><b>{formatPrice(promotion.salePriceCents)}</b></> : promotion.originalPriceCents > 0 ? <b>{formatPrice(promotion.originalPriceCents)}</b> : null}</div></div></section>; })}<div className="menu-layout"><section className="menu-categories"><button className="active" type="button">Frangos assados</button><button type="button">Bebidas</button><button type="button">Acompanhamentos</button><button type="button">Promoção do dia</button></section><section className="menu-empty"><Package size={32} /><h2>Cardápio pronto para configuração</h2><p>Os itens reais serão exibidos aqui após o proprietário preencher categorias, produtos, preços e disponibilidade no painel.</p></section><aside className="cart-summary"><p className="eyebrow">CARRINHO</p><h2>Seu pedido</h2><p>Adicione itens do cardápio para ver o resumo, o cupom e a entrega.</p><button onClick={() => go("checkout")} type="button">Ir para checkout <ArrowRight size={14} /></button></aside></div></main>;
}
