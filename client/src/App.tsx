...
export default function App() {
  ...
  return (
    <>
      ...
      <GoogleMetricsBootstrap />
      <CookieBanner />
    </>
  );
}

function GoogleMetricsBootstrap() {
  const configQuery = trpc.googleMetrics.publicConfig.useQuery(undefined, { retry: false });
  const [consent, setConsent] = useState<CookiePreference | null>(() => typeof window === "undefined" ? null : readCookiePreference(window.localStorage));
  useEffect(() => {
    const refreshConsent = () => setConsent(readCookiePreference(window.localStorage));
    window.addEventListener("prime-cookie-preference", refreshConsent);
    return () => window.removeEventListener("prime-cookie-preference", refreshConsent);
  }, []);
  useEffect(() => {
    const config = configQuery.data;
    if (!config?.searchConsoleVerification || typeof document === "undefined") return;
    const selector = 'meta[name="google-site-verification"]';
    let tag = document.head.querySelector<HTMLMetaElement>(selector);
    if (!tag) { tag = document.createElement("meta"); tag.name = "google-site-verification"; document.head.appendChild(tag); }
    tag.content = config.searchConsoleVerification;
  }, [configQuery.data]);
  useEffect(() => {
    const config = configQuery.data;
    if (consent !== "accepted" || !config?.isEnabled || typeof document === "undefined") return;
    const inject = (id: string, src: string) => {
      if (document.head.querySelector(`script[data-prime-metrics="${id}"]`)) return;
      const script = document.createElement("script"); script.async = true; script.src = src; script.dataset.primeMetrics = id; document.head.appendChild(script);
    };
    if (config.gaMeasurementId) {
      inject(`ga-${config.gaMeasurementId}`, `https://www.googletagmanager.com/gtag/js?id=${encodeURIComponent(config.gaMeasurementId)}`);
      const metricWindow = window as Window & { dataLayer?: unknown[]; gtag?: (...args: unknown[]) => void };
      metricWindow.dataLayer = metricWindow.dataLayer || [];
      metricWindow.gtag = (...args: unknown[]) => { metricWindow.dataLayer?.push(args); };
      metricWindow.gtag("js", new Date()); metricWindow.gtag("config", config.gaMeasurementId, { anonymize_ip: true });
    }
    if (config.gtmContainerId) inject(`gtm-${config.gtmContainerId}`, `https://www.googletagmanager.com/gtm.js?id=${encodeURIComponent(config.gtmContainerId)}`);
  }, [configQuery.data, consent]);
  return null;
}
...
function CheckoutPage({ go, cart, setCart }: { go: (view: PublicView) => void; cart: CartLine[]; setCart: React.Dispatch<React.SetStateAction<CartLine[]>> }) {
  ...
  const hostedStripeLinkQuery = trpc.payments.hostedStripeLink.useQuery(undefined, { retry: false });
...
type IntegrationProvider = "stripe" | "mercado_pago" | "pagbank" | "google_maps" | "whatsapp" | "email" | "assistant_ia";
...
const integrationCatalog: { provider: IntegrationProvider; title: string; text: string; label: string; webhook: boolean }[] = [
  ...
  { provider: "pagbank", title: "PagBank", text: "Checkout, Pix e notificações de pagamento.", label: "Token PagBank", webhook: true },
  ...
  { provider: "whatsapp", title: "WhatsApp + Evolution Go", text: "API pública, eventos de pedido e encaminhamento humano.", label: "Chave global da Evolution Go", webhook: true },
...
function Integracoes({ onNotice }: { onNotice: (message: string) => void }) {
  ...
  const saveMutation = trpc.integrations.saveCredential.useMutation({ onSuccess: (result) => { setSecret(""); setPaymentLink(""); setWebhookSecret(""); setSelected(null); statusQuery.refetch(); onNotice(`${result.provider}: credencial salva com segurança e exibida de forma mascarada.`); }, onError: () => onNotice("Não foi possível salvar a credencial. Entre como administrador e confira os dados informados.") });
  ...
  const submit = (event: React.FormEvent<HTMLFormElement>) => { event.preventDefault(); if (!selectedItem) return; saveMutation.mutate({ provider: selectedItem.provider, label: selectedItem.title, secret, paymentLink: selectedItem.provider === "stripe" ? paymentLink : undefined, webhookUrl, webhookSecret: webhookSecret || undefined }); };
  const isEvolutionGo = selectedItem?.provider === "whatsapp";
  const incomingEvolutionWebhook = typeof window === "undefined" ? "/api/webhooks/evolution" : `${window.location.origin}/api/webhooks/evolution`;
  return <section className="page-grid integrations-page"><div className="integration-intro"><Settings2 size={24} /><div><h2>Conexões seguras</h2><p>Cadastre cada chave por aqui. Depois do salvamento, o painel mostra apenas uma versão mascarada e o status da conexão.</p></div></div>{!canManage && <div className="integration-access-note"><ShieldCheck size={16} /> Entre com a conta administradora para consultar ou salvar credenciais de integração.</div>}<div className="integration-grid">{integrationCatalog.map((item) => { const itemStatus = configured.get(item.provider); return <article key={item.provider}><ShieldCheck size={18} /><h3>{item.title}</h3><p>{item.text}</p><small className={itemStatus?.isEnabled ? "integration-status configured" : "integration-status"}>{itemStatus?.isEnabled ? `Configurada: ${itemStatus.maskedSecret}` : "Ainda não configurada"}</small><button className="outline-button" type="button" onClick={() => { if (!canManage) { onNotice("Faça login como administrador para configurar integrações."); return; } setSelected(item.provider); }}>Configurar</button></article>; })}</div>{selectedItem && <form className="integration-form" onSubmit={submit}><div><p className="eyebrow">CONFIGURAÇÃO PROTEGIDA</p><h2>{selectedItem.title}</h2><p>O valor é cifrado no servidor. Não será mostrado novamente em texto completo.</p></div><label>{selectedItem.label}<input value={secret} onChange={(event) => setSecret(event.target.value)} type="password" autoComplete="off" required minLength={8} placeholder="Cole a chave privada" /></label>{isEvolutionGo ? <div className="integration-access-note"><ShieldCheck size={16} /><span><b>API Evolution Go validada.</b> Use a base pública <code>https://evo-go-evolution-go.xzhrcw.easypanel.host</code>; a rota de consulta é <code>/manager/instance/fetchInstances</code>. Para receber eventos na plataforma, configure no Evolution Go o endereço <code>{incomingEvolutionWebhook}</code> e envie a chave global no cabeçalho <code>apikey</code>.</span></div> : selectedItem.webhook && <><label>URL do webhook (opcional)<input value={webhookUrl} onChange={(event) => setWebhookUrl(event.target.value)} type="url" placeholder="https://seu-dominio.com/webhooks/..." /></label><label>Segredo de assinatura do webhook (opcional)<input value={webhookSecret} onChange={(event) => setWebhookSecret(event.target.value)} type="password" autoComplete="off" placeholder="Cole o segredo de assinatura" /></label></>}<div className="integration-form-actions"><button className="approve" disabled={saveMutation.isPending} type="submit">{saveMutation.isPending ? "Salvando…" : "Salvar com segurança"}</button><button className="outline-button" type="button" onClick={() => { setSelected(null); setSecret(""); setWebhookSecret(""); }}>Cancelar</button></div></form>}<GoogleMetricsSettings canManage={canManage} onNotice={onNotice} /><EmailDeliverySettings canManage={canManage} onNotice={onNotice} /></section>;
}

function GoogleMetricsSettings({ canManage, onNotice }: { canManage: boolean; onNotice: (message: string) => void }) {
  const utils = trpc.useUtils();
  const configQuery = trpc.googleMetrics.adminConfig.useQuery(undefined, { enabled: canManage, retry: false });
  const [gaMeasurementId, setGaMeasurementId] = useState(""); const [gtmContainerId, setGtmContainerId] = useState(""); const [searchConsoleProperty, setSearchConsoleProperty] = useState(""); const [searchConsoleVerification, setSearchConsoleVerification] = useState("");
  useEffect(() => { const config = configQuery.data; if (!config) return; setGaMeasurementId(config.gaMeasurementId || ""); setGtmContainerId(config.gtmContainerId || ""); setSearchConsoleProperty(config.searchConsoleProperty || ""); setSearchConsoleVerification(config.searchConsoleVerification || ""); }, [configQuery.data]);
  const saveMutation = trpc.googleMetrics.save.useMutation({ onSuccess: async () => { await Promise.all([utils.googleMetrics.adminConfig.invalidate(), utils.googleMetrics.publicConfig.invalidate()]); onNotice("Configurações Google salvas. A medição só é carregada após o consentimento de cookies."); }, onError: (error) => onNotice(error.message || "Não foi possível salvar as configurações Google.") });
  const submit = (event: React.FormEvent<HTMLFormElement>) => { event.preventDefault(); if (!canManage) return onNotice("Entre como administrador para salvar as configurações Google."); saveMutation.mutate({ gaMeasurementId, gtmContainerId, searchConsoleProperty, searchConsoleVerification }); };
  if (canManage && configQuery.isLoading) return <section className="email-delivery-settings"><div className="email-settings-heading"><div><p className="eyebrow">MÉTRICAS E SEO</p><h2>Carregando configurações Google…</h2><p>Buscando o status seguro de GA4, GTM e Search Console.</p></div><BarChart3 size={22} /></div></section>;
  if (canManage && configQuery.isError) return <section className="email-delivery-settings"><div className="email-settings-heading"><div><p className="eyebrow">MÉTRICAS E SEO</p><h2>Não foi possível carregar as configurações Google.</h2><p>As integrações existentes não foram alteradas. Tente consultar o status novamente.</p></div><BarChart3 size={22} /></div><div className="email-form-actions"><button className="outline-button" type="button" onClick={() => configQuery.refetch()}>Tentar novamente</button></div></section>;
  return <section className="email-delivery-settings"><div className="email-settings-heading"><div><p className="eyebrow">MÉTRICAS E SEO</p><h2>Google Analytics 4, Tag Manager e Search Console</h2><p>Cadastre somente identificadores públicos. GA4 e Tag Manager são carregados apenas depois que o visitante aceitar cookies de medição; a verificação do Search Console adiciona a meta tag técnica ao site.</p></div><BarChart3 size={22} /></div><form className="email-settings-form" onSubmit={submit}><div className="email-form-grid"><label>ID de medição GA4<input value={gaMeasurementId} onChange={(event) => setGaMeasurementId(event.target.value.toUpperCase())} placeholder="G-XXXXXXXX" /></label><label>ID do contêiner GTM<input value={gtmContainerId} onChange={(event) => setGtmContainerId(event.target.value.toUpperCase())} placeholder="GTM-XXXXXX" /></label><label>Propriedade Search Console<input type="url" value={searchConsoleProperty} onChange={(event) => setSearchConsoleProperty(event.target.value)} placeholder="https://seu-dominio.com/" /></label><label>Código de verificação Search Console<input value={searchConsoleVerification} onChange={(event) => setSearchConsoleVerification(event.target.value)} placeholder="Conteúdo da meta google-site-verification" /></label></div><div className="email-form-actions"><button className="approve" type="submit" disabled={!canManage || saveMutation.isPending}>{saveMutation.isPending ? "Salvando…" : "Salvar configurações"}<ShieldCheck size={15} /></button></div></form></section>;
}
...
function CookieBanner() { const [preference, setPreference] = useState<"loading" | CookiePreference | "pending">("loading"); useEffect(() => { const saved = readCookiePreference(window.localStorage); setPreference(saved ?? "pending"); }, []); const save = (choice: CookiePreference) => { setPreference(saveCookiePreference(window.localStorage, choice)); window.dispatchEvent(new Event("prime-cookie-preference")); }; if (preference !== "pending") return null; return <section className="cookie-banner" role="dialog" aria-label="Preferências de cookies"><Cookie size={22} /><div><h2>Preferências de cookies</h2><p>Cookies essenciais funcionam sempre. Aceite a medição para nos ajudar a melhorar sua experiência.</p></div><div className="cookie-actions"><button className="outline-button" onClick={() => save("necessary")} type="button">Usar somente necessários</button><button className="approve" onClick={() => save("accepted")} type="button">Aceitar medição</button></div></section>; }
