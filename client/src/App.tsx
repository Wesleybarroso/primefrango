...
function PublicPromotions() {
  ...
  if (promotion) return <div className="public-promotion-card"><p className="eyebrow">{promotion.badge || "PROMOÇÃO ATIVA"}</p><h3>{promotion.title}</h3><p>{promotion.description}</p><span>Vigência configurada pela loja</span></div>;
...
type PromotionForm = { title: string; description: string; badge: string; startsAt: string; endsAt: string; status: "draft" | "active" | "archived" };
const emptyPromotionForm: PromotionForm = { title: "", description: "", badge: "", startsAt: "", endsAt: "", status: "draft" };
...
function Promocoes({ onNotice }: { onNotice: (message: string) => void }) {
  ...
  const saveMutation = trpc.promotions.create.useMutation({ onSuccess: async () => { await refresh(); setForm(emptyPromotionForm); onNotice("Promoção criada. Ative-a quando estiver pronta para publicar."); }, onError: notifyError });
  const updateMutation = trpc.promotions.update.useMutation({ onSuccess: async () => { await refresh(); setEditingId(null); setForm(emptyPromotionForm); onNotice("Promoção atualizada com sucesso."); }, onError: notifyError });
  ...
  const save = (event: React.FormEvent<HTMLFormElement>) => { event.preventDefault(); const data = { ...form, startsAt: form.startsAt ? new Date(`${form.startsAt}T00:00:00`) : null, endsAt: form.endsAt ? new Date(`${form.endsAt}T23:59:59`) : null }; if (editingId) updateMutation.mutate({ id: editingId, data }); else saveMutation.mutate(data); };
  const edit = (promotion: NonNullable<typeof promotionsQuery.data>[number]) => { setEditingId(promotion.id); setForm({ title: promotion.title, description: promotion.description, badge: promotion.badge || "", startsAt: toDateInput(promotion.startsAt), endsAt: toDateInput(promotion.endsAt), status: promotion.status }); };
  const pending = saveMutation.isPending || updateMutation.isPending;
  return <section className="page-grid promotions-page"><div className="promotion-header"><div><p className="eyebrow">PROMOÇÕES DO DIA</p><h2>Crie e publique seus destaques.</h2><p>Uma promoção ativa aparece automaticamente na landing page e no cardápio durante a vigência definida.</p></div><span className="count-pill">{promotionsQuery.data?.length ?? 0} configuradas</span></div><form className="promotion-form" onSubmit={save}><div className="promotion-form-heading"><div><h2>{editingId ? "Editar promoção" : "Nova promoção"}</h2><p>Use somente informações comerciais reais que deseja anunciar.</p></div>{editingId && <button className="outline-button" type="button" onClick={() => { setEditingId(null); setForm(emptyPromotionForm); }}>Cancelar edição</button>}</div><label>Título da promoção<input required maxLength={120} value={form.title} onChange={(event) => setForm({ ...form, title: event.target.value })} placeholder="Ex.: Combo de domingo" /></label><label>Texto de apoio<textarea required maxLength={600} value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} placeholder="Explique o que está incluído e as condições." /></label><div className="promotion-form-row"><label>Selo curto (opcional)<input maxLength={48} value={form.badge} onChange={(event) => setForm({ ...form, badge: event.target.value })} placeholder="Ex.: SOMENTE HOJE" /></label><label>Status<select value={form.status} onChange={(event) => setForm({ ...form, status: event.target.value as PromotionForm["status"] })}><option value="draft">Rascunho</option><option value="active">Ativa</option><option value="archived">Arquivada</option></select></label><label>Início (opcional)<input type="date" value={form.startsAt} onChange={(event) => setForm({ ...form, startsAt: event.target.value })} /></label><label>Fim (opcional)<input type="date" value={form.endsAt} onChange={(event) => setForm({ ...form, endsAt: event.target.value })} /></label></div><button className="approve" disabled={pending} type="submit">{pending ? "Salvando…" : editingId ? "Salvar alterações" : "Criar promoção"}</button></form><section className="promotion-list"><div className="card-heading"><div><h2>Promoções configuradas</h2><p>Controle a publicação sem precisar alterar código.</p></div></div>{promotionsQuery.isLoading && <p className="empty-caption">Carregando promoções…</p>}{!promotionsQuery.isLoading && !promotionsQuery.data?.length && <p className="empty-caption"><Tags size={16} /> Nenhuma promoção configurada ainda.</p>}{promotionsQuery.data?.map((promotion) => <article className="promotion-item" key={promotion.id}><div><span className={`promotion-status ${promotion.status}`}>{promotion.status === "active" ? "Ativa" : promotion.status === "draft" ? "Rascunho" : "Arquivada"}</span><h3>{promotion.title}</h3><p>{promotion.description}</p><small>{promotion.badge || "Sem selo"} · {promotion.startsAt ? `a partir de ${new Date(promotion.startsAt).toLocaleDateString("pt-BR")}` : "sem início definido"}{promotion.endsAt ? ` · até ${new Date(promotion.endsAt).toLocaleDateString("pt-BR")}` : ""}</small></div><div className="promotion-actions"><button className="outline-button" type="button" onClick={() => edit(promotion)}>Editar</button><button className="approve" type="button" onClick={() => statusMutation.mutate({ id: promotion.id, status: promotion.status === "active" ? "archived" : "active" })}>{promotion.status === "active" ? "Encerrar" : "Ativar"}</button><button className="danger-button" type="button" onClick={() => { if (window.confirm("Remover esta promoção?")) removeMutation.mutate({ id: promotion.id }); }}>Remover</button></div></article>)}</section></section>;
}
